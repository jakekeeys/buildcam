...
func (t *Timelapse) updateLapse(filter, lapseName, fps string) error {
	...
	_, err = exec.Command("ffmpeg", "-framerate", fps, "-pattern_type", "glob", "-i", filter, "-s:v", "1920x1080", "-c:v", "libx264", "-crf", "24", "-maxrate", "800k", "-pix_fmt", "yuv420p", lapseTmpPath).CombinedOutput()
	if err != nil {
		return err
	}
